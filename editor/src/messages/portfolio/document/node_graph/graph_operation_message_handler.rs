...
fn usvg_colour(c: usvg::Color, a: f32) -> Color {
...
fn apply_usvg_stroke(stroke: &Option<usvg::Stroke>, modify_inputs: &mut ModifyInputsContext) {
	if let Some(stroke) = stroke {
		if let usvg::Paint::Color(color) = &stroke.paint {
			modify_inputs.stroke_set(Stroke {
				color: Some(usvg_colour(*color, stroke.opacity.get())),
...
fn apply_usvg_fill(fill: &Option<usvg::Fill>, modify_inputs: &mut ModifyInputsContext) {
	if let Some(fill) = &fill {
		modify_inputs.fill_set(match &fill.paint {
			usvg::Paint::Color(color) => Fill::solid(usvg_colour(*color, fill.opacity.get())),
			usvg::Paint::LinearGradient(linear) => Fill::Gradient(Gradient {
				...
				positions: linear.stops.iter().map(|stop| (stop.offset.get() as f64, Some(usvg_colour(stop.color, stop.opacity.get())))).collect(),
			}),
			usvg::Paint::RadialGradient(radial) => Fill::Gradient(Gradient {
				...
				positions: radial.stops.iter().map(|stop| (stop.offset.get() as f64, Some(usvg_colour(stop.color, stop.opacity.get())))).collect(),
			}),

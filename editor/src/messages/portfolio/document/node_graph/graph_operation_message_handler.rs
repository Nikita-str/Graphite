	...
	if let Some(fill) = &fill {
		modify_inputs.fill_set(match &fill.paint {
			...
			usvg::Paint::LinearGradient(linear) => Fill::Gradient(Gradient {
				...
				positions: linear.stops.iter().map(|stop| (stop.offset.get() as f64, usvg_colour(stop.color, stop.opacity.get()))).collect(),
			}),
			usvg::Paint::RadialGradient(radial) => Fill::Gradient(Gradient {
				...
				positions: radial.stops.iter().map(|stop| (stop.offset.get() as f64, usvg_colour(stop.color, stop.opacity.get()))).collect(),
			}),
